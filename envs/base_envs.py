...
class BaseEnv(gym.Env):
    ...
    def _init_data(
        ...
        # GET FACTUAL EPISODE LENGTH
        len_data = len(data["Date"])
        if self.len_episode != len_data:
            warnings.warn(f'Length of datafile provided ({len_data}) does not match modeling period implied from '
                          f'environment parameters provided ({self.len_episode})')
        self.len_episode = min(self.len_episode, len_data)
...
class BatteryBaseEnv(BaseEnv):
    ...
    def _init_storage(self):
        ...
        # Storage with complex degradation -> Not properly tested yet
        # elif self.storage_dict['degradation']['type'] == 'dynamic':
        #     storage = DynamicDegradingBESS(**self.storage_dict,
        #                                    resolution_h=self.resolution_h,
        #                                    tracking=True)
